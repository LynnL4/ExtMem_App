...
// Log define
#define CAMERA_INFO(fmt, arg...) LOG("<<-CAMERA-INFO->> " fmt "\n", ##arg)
#define CAMERA_ERROR(fmt, arg...) LOG("<<-CAMERA-ERROR->> " fmt "\n", ##arg)
#define CAMERA_DEBUG(fmt, arg...)                               \
  do                                                            \
  {                                                             \
    if (CAMERA_DEBUG_ON)                                        \
      LOG("<<-CAMERA-DEBUG->> [%d]" fmt "\n", __LINE__, ##arg); \
  } while (0)
#define CAMERA_DEBUG_ARRAY(array, num)   \
  do                                     \
  {                                      \
    int32_t i;                           \
    uint8_t *a = array;                  \
...
#define CAMERA_DEBUG_FUNC()                                           \
  do                                                                  \
  {                                                                   \
    if (CAMERA_DEBUG_FUNC_ON)                                         \
      LOG("<<-CAMERA-FUNC->> Func:%s@Line:%d\n", __func__, __LINE__); \
  } while (0)
...
typedef struct
{
	...
	uint32_t width;
	uint32_t height;
	//osSemaphoreId_t sem_showHandle;
}Frame_TypeDef;
